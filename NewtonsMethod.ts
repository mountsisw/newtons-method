...
class newtonsMethod
{
    ...
    private constOne: complexNumber = new complexNumber(1, 0);
    private constExponent:  complexNumber;

    constructor(private exponent: number, private maxAttempts: number, private coefficient: complexNumber = new complexNumber(1, 0))
    {
        let nLoop: number, angle: number;
    ...
    {
        let nLoop: number;
        let xN: complexNumber = point;
        let xNMinusOne: complexNumber;
        for (nLoop = 2; nLoop <= this.exponent; nLoop++)
        {
            xNMinusOne = xN;
            xN = xN.multiply(point);
        }
        let numerator: complexNumber = xN.subtract(this.constOne);
        let denomonator: complexNumber = xNMinusOne.multiply(this.constExponent);
...
function drawFractalRoots(exponentIndex: number) : void
{
    if (exponentIndex < exponents.length)
    {
        let maxAttempts:number = gradients[gradients.length - 1];
        let algo: newtonsMethod = new newtonsMethod(exponents[exponentIndex], maxAttempts, new complexNumber(1, 0));
...
function drawFractalRootsGradients(exponentIndex: number, gradientIndex: number) : void
{
    ...
    else setTimeout(drawFractalRoots, 0, exponentIndex + 1);
}
